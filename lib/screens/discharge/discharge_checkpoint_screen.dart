...
class DischargeCheckpointScreen extends ConsumerWidget {
  final String admissionId;
  const DischargeCheckpointScreen(
      {super.key, required this.admissionId});
  ...
  Color _statusColor(WorkupStatus status) {
    switch (status) {
  ...
  Future<void> _discharge(BuildContext context, WidgetRef ref) async {
    ...
    final admission = await ref.read(admissionByIdProvider(admissionId).future);
